...
8. **Strength + mobility** — open `/fuerza` for the Fuerza de Papá plan: functional goals, exercise technique, weekly load, pain/priority filtering, mobility drills, functional tests, movement progressions, and an editable wooden mannequin. Logged sessions are completed personal tasks tagged under `ejercicio>fuerza-papa`, so they also appear in `/habits`.
