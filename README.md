...
8. **Strength + mobility** — open `/fuerza` for the Fuerza de Papá plan: functional goals, exercise technique, weekly load, pain/priority filtering, mobility drills, functional tests, movement progressions, and an editable wooden mannequin with an articulated pose rig. Exercise mannequins use a green/yellow/red relative-effort map instead of a binary muscle highlight. When logging training, navigate to any past day, select completed exercises or manual muscles, review their combined heatmap, then save or update that day's accumulated session. Logged sessions are completed personal tasks tagged under `ejercicio>fuerza-papa`, so they also appear in `/habits`.
